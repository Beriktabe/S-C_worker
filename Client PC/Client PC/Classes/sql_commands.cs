...
namespace Beriktabe.SqlWorker
{
    class sql_commands
    {
        ...
        public static void ConnectToFile(string path)
        {
            connection = new SQLiteConnection("Data source = " + path);
        }

        public static string[] Get(string name, string table)
        {
            string nm = "'" + name + "'";
            string tbl = "'" + table + "'";
            List<string> mass = new List<string>();
            SQLiteCommand command = new SQLiteCommand(connection);
            connection.Open();
            command.CommandText = string.Format("SELECT {0} FROM {1}", nm, tbl);
            using (SQLiteDataReader rdr = command.ExecuteReader())
            {
                while (rdr.Read())
                {
